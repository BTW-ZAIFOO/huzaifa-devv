    ...
    const renderMessageStatus = (message) => {
        ...
        return (
            <span className="ml-1">
                ...
                {message.status === "read" && (
                    <i className="fas fa-check-double text-blue-300 text-xs" title="Read"></i>
                )}
            </span>
        );
    };
        ...
        if ((isBannedUser || isBlockedUser) && !isAdmin) {
            return (
                <div className="flex justify-center mb-4">
                    <div className={`${isBannedUser ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"
                        } px-4 py-2 rounded-lg text-sm text-center max-w-md shadow-sm`}>
                        <i className={`fas ${isBannedUser ? "fa-ban" : "fa-exclamation-triangle"} mr-2`}></i>
                        {isBannedUser
        ...
        <div className="flex-1 flex flex-col bg-white/90 rounded-br-3xl shadow-inner">
            ...
            <form onSubmit={handleSendMessage} className="p-6 border-t flex items-end gap-3 bg-white/95 shadow-lg rounded-b-3xl">
                <div className="flex-1 relative">
                    ...
                    <button
                        type="button"
                        className={`absolute right-3 bottom-3 p-2 rounded-full focus:outline-none transition-all ${isRecording
                ...
                <button
                    type="submit"
                    className={`p-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-full flex-shrink-0 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 transition-all ${isMessageDisabled || (!messageText.trim() && !isRecording)
