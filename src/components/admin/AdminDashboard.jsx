...
const AdminDashboard = ({
  ...
  const getUserCount = (condition) =>
    Array.isArray(users) ? users.filter(condition).length : 0;
  const totalUsers = getUserCount((user) => user.role !== "admin");
  const onlineUsers = getUserCount(
    (user) => user.status === "online" && user.role !== "admin"
  );
  const offlineUsers = getUserCount(
    (user) =>
      user.status !== "online" &&
      user.status !== "blocked" &&
      user.status !== "banned" &&
  ...
  const flaggedContent = getUserCount(
    (user) =>
      ...
      <div className="flex-1 p-6 bg-gray-50 flex flex-col items-center justify-center">
        <div className="bg-white rounded-lg p-8 shadow text-center max-w-md">
          ...
          <p className="text-gray-500 mb-4">
            There are currently no users in the system or the user data could
            not be loaded.
          </p>
        </div>
      </div>
  ...
  if (Array.isArray(filteredUsers)) {
    filteredUsers.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "flags":
          const aFlags =
            (a.flaggedWords?.length || 0) +
            (flaggedUsers[a._id || a.id]?.length || 0);
          const bFlags =
            (b.flaggedWords?.length || 0) +
            (flaggedUsers[b._id || b.id]?.length || 0);
          compareA = aFlags;
          compareB = bFlags;
          break;
        default:
  ...
  const handleReportUserClick = (userId) => {
    ...
    if (user.isReported) {
      ...
      if (reason && reason.trim() !== "") {
        ...
        toast.success(
          `Report submitted for ${user.name} with reason: "${reason}"`,
          {
            autoClose: 5000,
          }
        );
                ...
                {Array.isArray(filteredUsers) && filteredUsers.length > 0 ? (
                  filteredUsers.map((user) => {
                    ...
                    return (
                      <tr
                        key={userId}
                        className={`hover:bg-gray-50 ${getBorderStyle()}`}
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex-shrink-0 h-10 w-10 relative">
                              ...
                              <span
                                className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${user.status === "online"
                                    ? "bg-green-500"
                                    : user.status === "blocked"
                                      ? "bg-red-500"
                                      : user.status === "banned"
                              ...
                              <div className="text-sm font-medium text-gray-900 flex items-center">
                                ...
                                {user.bannedBy && (
                                  <span className="ml-2 bg-black text-white text-xs px-1.5 py-0.5 rounded-full inline-flex items-center">
                                    <i className="fas fa-gavel text-xs mr-1"></i>{" "}
                                    Banned
                                  </span>
                                )}
                              </div>
                              ...
                              <div className="mt-1 text-xs text-gray-600">
                                {userFlaggedWords
                                  ...
                                  .map((word, idx) => (
                                    <span
                                      key={idx}
                                      className="bg-red-50 text-red-700 px-1 py-0.5 rounded mr-1"
                                    >
                                      {word}
                                    </span>
                                  ))}
                                {userFlaggedWords.length > 3 && (
                                  <span className="text-xs text-gray-500">
                                    +{userFlaggedWords.length - 3} more
                                  </span>
                                )}
                              </div>
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                          ...
                          >
                            <i
                              className={`fas ${user.status === "blocked"
                                  ? "fa-unlock"
                                  : "fa-ban"
                                }`}
                            ></i>
                          ...
                          <button
                            ...
                            className={`${user.status === "banned"
                                ? "text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100"
                                : "text-gray-700 hover:text-black bg-gray-50 hover:bg-gray-100"
                              } p-1 rounded`}
                            title={
                              user.status === "banned"
                                ? "Unban User"
                          ...
                          >
                            <i
                              className={`fas ${user.status === "banned"
                                  ? "fa-user-check"
                                  : "fa-user-slash"
                                }`}
                            ></i>
                          ...
                          <button
                            ...
                            title={
                              user.isReported
                                ? `Remove report: "${user.reportReason || "No reason provided"
                                }"`
                                : "Report User"
                            }
                          >
                            <i
                              className={`fas ${user.isReported
                                  ? "fa-flag-checkered"
                                  : "fa-flag"
                                }`}
                            ></i>
          ...
          {filteredUsers.length > 0 && filteredUsers.length > 10 && (
            <div className="flex justify-between items-center mt-4">
              ...
              <div className="space-x-1">
                ...
                <button className="px-3 py-1.5 bg-blue-50 border border-blue-200 rounded text-blue-700">
                  1
                </button>
                <button className="px-3 py-1 bg-white border border-gray-300 rounded text-gray-700 hover:bg-gray-50">
...
const StatCard = ({ title, value, icon, color, onClick, active }) => {
  ...
  return (
    ...
    >
      <div>
        <div className={`text-sm font-medium mb-1 ${classes.text}`}>
          {title}
        </div>
        <div className={`text-2xl font-bold ${classes.value}`}>{value}</div>
      </div>
      <div
        className={`w-12 h-12 rounded-lg ${classes.icon} flex items-center justify-center ${classes.text}`}
      >
